...
<form id="user_regist_form" action="<%= request.getContextPath() %>/regist.khs" method="post">
	<table>
		...
		<tr>
			<th>ID(*)</th>
			<td><input type="text" name="user_id" placeholder="1~10자 문자를 입력하세요. (필수입력, 중복체크 필수)" required> 
			<input type="button" id="idCheckBtn" value="ID 중복체크">
			<span id="checkIDArea"></span>
			</td>
		</tr>
		<tr>
			<th>PW(*)</th>
			<td><input type="password" name="user_pw" placeholder="1~15자 문자를 입력하세요. (필수입력사항)" required>
			<span id="checkPWArea"></span></td>
		</tr>
		<tr>
			<th>PW 확인(*)</th>
			<td><input type="password" name="user_pw_confirm" required>
			<span id="checkPWConfirmArea"></span></td>
		</tr>
		<tr>
			<th>닉네임(*)</th>
			<td><input type="text" name="user_nick" value="${ param.user_nick }" placeholder="1~8자 문자를 입력하세요. (필수입력, 중복체크 필수)" required > 
			<input type="button" id="nickCheckBtn" value="닉네임 중복체크">
			<span id="checkNickArea"></span>
			</td>
		</tr>
		<tr>
			<th>연락처</th>
			<td><input type="number" name="user_tel" value="${ param.tel }" placeholder="1~11자 숫자를 입력하세요. (선택입력사항)" >
			<span id="checkTelArea"></span>
			</td>
		</tr>
		<tr>
			<th>이메일</th>	
			<td><input type="text" name="user_mail" value="${ param.address }" placeholder="1~30자 문자를 입력하세요. (선택입력사항)">
			<span id="checkMailArea"></span>
			</td>		
		</tr>		
		<tr>
			<th colspan="2"><input type="submit" name="submit" value="가입하기"></th>
		</tr>
	</table>
...
<p><a href='<%= request.getContextPath() %>/main.khs'>메인화면으로 이동</a></p>
